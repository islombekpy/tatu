...
.then(data => {
  questionElements.forEach((questionEl, index) => {
    const answerData = data[index];
    const answer = answerData?.answer || "-";
    const answerIndex = answerData?.index || null;
    ...
    if (answerIndex !== null) {
      finalAnswer = `t${answerIndex}: ${answer}`;
    }

    const randomPosition = Math.floor(Math.random() * (questionText.length + 1));
    questionTextElement.textContent = questionText.slice(0, randomPosition) + finalAnswer + questionText.slice(randomPosition);
    questionEl.setAttribute("title", finalAnswer);
  });
})
